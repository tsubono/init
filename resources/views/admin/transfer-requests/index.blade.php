            ...
            <div class="p-admin-list__infos tab-content pt-25px">
                <div>
                    @forelse ($transferRequests as $index => $transferRequest)
                                    ...
                                    <div class="modal-body">
                                        <div class="text-center">
                                            @if ($transferRequest->adviserUser->payment_method === 'Paypal送金')
                                                <h2 class="p-heading2 mt-0 text-center">Paypalメールアドレス</h2>
                                                <p class="m-5 text-large">{{ $transferRequest->adviserUser->paypal_email }}</p>
                                            @elseif ($transferRequest->adviserUser->payment_method === '口座振替')
                                                <h2 class="p-heading2 mt-0 text-center">口座情報</h2>
                                                <p class="my-2 p-label w-25 mx-auto">表面</p>
                                                <img src="{{ $transferRequest->adviserUser->account_image_1  }}" alt="口座情報" />
                                                <p class="my-2 mt-3 p-label w-25 mx-auto">裏面</p>
                                                <img src="{{ $transferRequest->adviserUser->account_image_2  }}" alt="口座情報" />
                                            @else
                                                <p class="p-error-text">※ 支払い方法が登録されていません</p>
                                            @endif
                                        </div>
                                    </div><!-- /.modal-body -->
                                </div><!-- /.modal-content -->
                        ...
                        <div class="modal p-modal p-setting fade" id="updateModal{{ $index }}" tabindex="-1" aria-labelledby="updateModalLabel{{ $index }}">
                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                <div class="modal-content">
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="閉じる"></button>
                                    <div class="modal-body">
                                        <h2 class="p-heading2 mt-0 text-center">更新確認</h2>
                                        <p class="text-center">
                                            「{{ $transferRequest->adviserUser->full_name }}」さんの振込申請ステータスを<br>
                                            振り込み済みに更新します。<br>よろしいですか？
                                        </p>
                                        <form action="{{ route('admin.transfer-requests.update-status', compact('transferRequest')) }}" method="post">
                                            @csrf
                                            <button class="p-btn p-btn__defalut">更新する</button>
                                        </form>
                                    </div><!-- /.modal-body -->
                                </div><!-- /.modal-content -->
                            </div><!-- /.modal-dialog -->
                        ...
                        <!-- /振り込み済みにするモーダル -->
                    @empty
                        <div class="text-center">振り込み申請は登録されていません。</div>
                    @endforelse
                </div><!--/.tab-pane -->
